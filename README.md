...
- Retry-safe Session client operation identity for Minimal Host and full-product adapters
...
- `@agent-workbench/platform/features/session-branch`
- `@agent-workbench/platform/features/turn-queue`
...
Internal activity and user-visible result state remain separate across that boundary. A Host may pass `activityKind` and an optional `activityLabel`; context compaction is shown as “整理上下文” above the Composer, suppresses steer, and makes “下一轮” the sole send action. The client retains the active Turn's result-bearing classification so a compact completion with incomplete metadata neither creates a new result nor clears an existing unread result. Recoverable WebSocket startup failures return the connection to `stopped`, allowing the next request to reconnect on the same object; only explicit owner `close()` is terminal.

The built-in Minimal Host initially reads 50 Session summaries and the latest five Turns for the selected Session, prepends ten earlier Turns per request, and reads one Turn's technical items only when expanded. Image bytes resolve only near the viewport or on click, and live browser deltas are committed to React state at most once per animation frame.

`SessionBrowser` and `SessionWorkspace` provide the standard Session search/list/archive interaction, cursor-driven list loading, transcript and earlier-message paging, lazy technical-detail loading, queued turns, requests, Composer, click/drag/paste attachment upload, model/reasoning/access selectors, model-advertised Fast service tier, user-message Edit/Fork actions, status, Sub Agent, and Realtime UI. While a Turn is running, its stop control sits in the Composer action row immediately before “下一轮”, keeping the execution action next to the queue/steer controls in both full products and the Minimal Host. Time grouping uses incremental pagination; context and attention grouping continue loading lightweight pages until the grouped index is complete. `SessionList` exposes the exact same list implementation for products that already own the Session detail surface. The shared list supports consumer-provided context/time/status grouping, rich execution states, favorites, separate end and archive actions, host filter extensions, and product-owned full-text-search/history entry points. Shared transcript rendering supports safe Markdown for both user and Agent messages, Codex `\\[...\\]` / `\\(...\\)` math through KaTeX, and both the legacy `::codex-inline-vis` directive and current `visualize` content reference; directives inside fenced or indented code remain literal examples. Products resolve visualization paths through `actions.visualizationUrl` and remain responsible for their own allowlist. Local absolute and `./` / `../` file links stay delegated to the host for authorized in-page preview; absolute paths use a `file:///...` browser href instead of being resolved against the Workbench origin. They expose an overlaid folder reveal button only while the link is hovered or keyboard-focused, so the hidden action never reserves inline text space; products opt in with `actions.onRevealLink`, keep path authorization and operating-system behavior in the host, and may override the label with `labels.revealFile`. Agent commentary stays in the transcript while its Turn is running; after the Turn reaches a terminal status, consecutive commentary messages in that Turn share one compact disclosure without mixing command or tool details into the conversation. File selection uses the browser input event and snapshots its live `FileList` before resetting the field, so the current upload is retained and the same file can be selected again. Pasted clipboard files and images become attachments. The Composer is always a direct text editor and never replaces the draft with a formatted preview. Ordinary rich clipboard wrappers use their exact plain-text value; by default, headings, lists, tables, fenced code, blockquotes, or multi-paragraph structures become a timestamped Markdown attachment, while long unstructured text becomes a plain-text attachment at 1,000 characters or before exceeding the Composer limit. Consumers that prefer all short text to stay editable may set `attachmentPolicy.structuredTextPaste` to `inline`; the long-text limit and clipboard file/image behavior remain unchanged. Platform owns the versioned attachment envelope, preserves stable attachment identity and metadata in the transcript, and exposes host-owned attachment/image open actions; products still own file storage, preview URLs and Session/path authorization. `actions.onUploadAttachments` may report each file's progress through the optional `onProgress` callback, while failed cards remain retryable without blocking a text-only submission. The shared file drawer previews image, PDF, audio, spreadsheet, Markdown, code, text, and sandboxed HTML; image documents use a centered lightbox, while managed Resource previews are labelled as Session artifacts and hosts opt into local edit/reveal/external actions. With Resources enabled, the built-in Minimal Host resolves authorized uploaded images into inline user-message thumbnails that open in that lightbox, and promotes completed provider-native generated images into durable final-answer media without a separate image flag. Technical items can expose host-owned `artifacts` through open/reveal actions. The shared Playwright proxy pins its child process to the configured output directory and rewrites generated artifact links to absolute local paths, so screenshots and PDFs do not inherit an unrelated consumer working directory. Any explicit upward wheel, touch, or scrollbar movement pauses automatic transcript following immediately, including inside the former 200-pixel tolerance; streaming updates then show the floating “有新消息” shortcut without taking over the reader's position. Reaching the latest region or activating the shortcut resumes following. Products supply the model catalog and implement `actions.onExecutionProfileChange` to apply execution settings through their Runtime adapter. `SideChatPanel` and `SubagentPanel` provide shared auxiliary surfaces. `SessionBranchController` plans Edit/Fork without product objects, and `SessionTurnQueue` plus its dispatcher own durable FIFO/recovery semantics; consumers still supply stores, Runtime calls, authorization, project linking and navigation. `messageEdit`, `messageFork`, `queuedTurns`, and `steer` are independent Session feature flags, while Runtime capabilities remain the final execution gate. Products mark eligible messages with `canEdit` / `canFork`, implement the corresponding actions, and supply grouping labels, archive and paging actions. The Minimal Host keeps those branch actions available for owned portable history from an earlier Run by projecting retained history into a fresh Runtime, while shared Sessions remain read-only. They can append product-specific content after a standard message with `extensions.renderAfterMessage`; Core does not know about projects, analysis cards, reports, or artifact canvases.
...
Minimal Host optionally accepts a consumer-configured, Gateway-verified shared-Session access header. It merges explicitly scoped read-only Sessions into the standard list, denies mutations and event streams, reauthorizes managed-resource reads, and supports an idempotent safe continuation into a new owner Session and fresh Runtime without reusing the source provider thread. The stable boundary is recorded in [`docs/architecture.md`](docs/architecture.md); accounts, Share/link persistence, revocation, audit and authorization-envelope paging remain consumer responsibilities.
