...
`FilesystemResourceStore` now supplies the product-neutral default storage mechanics: public `resource/v1` descriptors, staged uploads, Session authorization, atomic commit after Runtime acceptance, private content keys, usage inspection, and dry-run collection planning. Consumers still choose the physical root or an alternative backend and own preview/download authorization, backup, encryption, quotas, and retention overrides.
