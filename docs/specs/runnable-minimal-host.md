...
Lifecycle: implemented contract with unresolved provider/retention decisions. The project-free Host, offline strong-isolation base, immutable Runtime-enforced Skill allowlist, immutable trusted read-only MCP packages, fixed Codex model broker, read-only adapter sidecars, independent Data Skill Lab Candidate/Baseline parity, and legacy-host retirement are complete. This spec does not define a Data Skill evaluation product.
...
- provider-discovered model and reasoning options plus an optional Environment `runtime.accessModes` allowlist. Each Session persists `{model, reasoningEffort, accessMode, serviceTier}` and the Host applies it consistently to first attach, resume, queued Turns and Edit/Fork branches. Updates are owner-only, idle-only and validated against the current Runtime catalog and Profile policy;
...
Running and waiting do not replace the shared Composer with a reduced Host-specific layout. Attachments and supported Turn actions remain visible. Editable model, reasoning, permission and service-tier controls collapse into `执行设置 ⓘ`; activating it opens a read-only anchored bubble in a fixed overlay layer, so showing or hiding details does not resize the Composer, footer or transcript. Completed commentary remains available through the shared collapsible process presentation.

The Observer lists Sessions across owners. Its default page excludes archived Sessions so retained release checks and replaced Edit sources do not obscure current operational work; an explicit archive-history toggle restores them without deleting audit evidence. The page shows one Turn timeline with persisted user/assistant messages, externally emitted commentary, and expandable structured execution steps. New Runtime items retain the available tool or command name, redacted input, result or error, and reported duration; older records remain readable but cannot recover fields that were never persisted. Fixed Run stdout/stderr tails are exposed only through a separate bounded diagnostic API. The Observer is read-only, enforces the normal Host token plus the configured observer header, removes known credentials and Run-root paths, and streams only change notifications before the client refetches the redacted projection. The observer contract does not expose hidden chain-of-thought or arbitrary files. A Consumer gateway owns administrator authorization and must strip any browser-supplied observer header before adding its trusted claim.

When `features.attachments` is enabled, the Minimal Host stages uploaded files through the Platform `ResourceStore` under the effective Session resource root. That is the current Run's private `resources` root by default or the consumer-bound portable root when configured. A Turn may reference only resources previously uploaded to that same Session; Runtime acceptance commits them as Session-durable resources and the persisted transcript contains normalized descriptors, not host paths. A rejected Turn leaves its staged resources recoverable for retry. The first accepted user message also replaces the default Session title while preserving titles explicitly supplied by a consumer. Edit clones retained history and the selected message's managed attachments, submits the edited prompt as a new Turn and archives the source after success. Fork clones the transcript and managed attachments through the selected Turn, creates a continuation Runtime, does not submit a Turn until the user asks something new and keeps the source active.

The same attachment capability enables image presentation without a second image flag. Uploaded image attachments open in the shared in-page lightbox. Completed provider-native `imageGeneration` results are staged as transient Run output, promoted to Session-durable artifacts, attached only to the final Agent answer and read through the same owner-scoped content route. Raw base64 and provider paths never enter the Session document. Generated media survives refresh and is cloned with retained messages during Edit, Fork and shared continuation.

Portable persistence deliberately does not make Runtime threads portable. Session identity, transcripts, drafts and managed Resources are durable facts; `session-runtime.json`, Runtime homes, credentials and queued Turns remain inside one Run. A new Run reads an owned earlier-Run Session without side effects and presents it as an ordinary writable Session. Attachment/directory staging remains available without starting a Runtime. On the first new Turn, the Host serializes preparation for that Session, creates one fresh current-Run Runtime binding and supplies bounded retained transcript as untrusted conversation context instead of reusing the stale provider thread. The accepted Turn consumes that context exactly once; attachment authorization is checked again against the same portable Session. Preparation failure removes an incomplete current-Run binding while leaving the durable Session, draft, staged Resources and idempotency retry available. Edit and Fork retain their existing branch semantics. Shared recipients keep the separate read-only continuation flow, and a GET must never create a blank replacement thread.
...
The independent Data Skill Lab owns only its Profile builder, CLI defaults, evaluation inputs and evidence. Its domain defaults are not Platform source; Personal server, project state, `PAW_*` contracts and Personal Runtime Home are not part of the Lab or target architecture.
