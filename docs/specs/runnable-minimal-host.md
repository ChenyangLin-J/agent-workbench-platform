...
Portable persistence deliberately does not make Runtime threads portable. `sessions.json` and managed Resources are durable facts; `session-runtime.json`, Runtime homes and queued Turns remain inside one Run. When a new Run reads a Session created by an earlier Run, list/detail remain available, direct Composer and mutation routes return `HOST_SESSION_CONTINUATION_REQUIRED`, and no Runtime thread is created. If Edit or Fork is enabled, the owner may branch an eligible retained user message: the Host creates a fresh current-Run Runtime and supplies bounded retained transcript as untrusted conversation context instead of reusing the stale provider thread. Edit starts the replacement Turn immediately and archives the source only after that succeeds. Fork copies through the selected Turn, leaves the source active, and defers the retained transcript and selected-message attachment inputs until the user's first new prompt. Shared recipients keep the separate read-only continuation flow, and a GET must never create a blank replacement thread.
