...
Image provenance remains explicit across this boundary. Provider-native generated-image result items may be promoted and attached to the final Agent message. A final Agent message that explicitly references an authorized image may publish that image through the same result-media contract. Images returned only to let the Agent inspect state, including screenshot and image-view tool observations from the parent Agent or a Subagent, remain technical process and must not be inferred as final output from their order, count, or proximity to the final answer.
