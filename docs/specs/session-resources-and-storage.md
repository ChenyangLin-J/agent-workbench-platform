...
Image provenance remains explicit across this boundary. The built-in Minimal Host promotes completed provider-native generated-image result items and attaches their Resource descriptors to the final Agent message; other Hosts may use the same contract. A final Agent message that explicitly references an authorized image may publish that image through the same result-media contract. Images returned only to let the Agent inspect state, including screenshot and image-view tool observations from the parent Agent or a Subagent, remain technical process and must not be inferred as final output from their order, count, or proximity to the final answer.
