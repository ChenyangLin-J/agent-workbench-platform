...
- Composer remains a direct text editor and never substitutes a formatted preview for the user's draft. Ordinary clipboard HTML wrappers and a standalone copied heading use the exact plain-text value; heading styling alone is not complex structure. Multi-block content, lists, tables, code blocks and other structurally complex clipboard content become an attachment through the existing staged Resource flow instead of adding a second editing mode.
