...
- A consumer may set bootstrap `sessionStart` to `new`. With no explicit Session deep link, the Minimal Host then creates and selects an empty persisted Session after the first list load so the Composer is immediately available; a valid `?session=` target still wins. Empty Session creation remains idempotent and does not start a Runtime thread before the first Turn.
