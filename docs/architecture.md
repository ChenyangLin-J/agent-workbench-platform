...
- Transcript auto-follow is conditional on reading intent. An upward wheel, touch gesture, or observed scroll-position decrease pauses following before the reader passes the general distance threshold; later streaming deltas preserve that viewport and surface the existing new-message shortcut. Reaching the latest region or activating the shortcut resumes following. Explicit submission still follows the new user message, and viewport resize follows only until the user interrupts that submission-follow state.
