...
- A consumer may set bootstrap `sessionStart` to `new` to guarantee an immediately usable Session without forcing a new one on every entry. With no explicit Session deep link, the Minimal Host selects the newest existing Session; only an empty list creates and selects one persisted blank Session. A valid `?session=` target still wins. Empty Session creation remains idempotent and does not start a Runtime thread before the first Turn.
...
- A Minimal Host Session may persist a public execution profile containing only model, reasoning effort, Profile-authorized access mode and service tier. The Host obtains the model catalog through the Runtime provider, validates every update against that live catalog and the Environment `runtime.accessModes` allowlist, and applies the same profile to first attach, resumed Sessions, queued Turns and Edit/Fork branches. Historical Profiles without `accessModes` retain their existing single effective mode. While a Session is running or waiting, the shared Composer keeps attachments and Turn controls visible but replaces editable execution controls with the fixed `执行设置 ⓘ` entry; its read-only fixed-position bubble does not participate in Composer or transcript layout.
- `@agent-workbench/platform/session-client` supplies product-neutral browser application state. Its operation controller retains mutation identity for uncertain retries; its reconciliation helpers merge authoritative snapshots with optimistic/live items; and its event controller owns active-Turn, queue, request, delta, reconnect and polling transitions. The event controller retains result-bearing classification for an active Turn, so an internal completion with incomplete metadata cannot change the unread projection. Transport, authentication, product events, extension recovery, persistence and user-facing error policy remain consumer-owned callbacks.
- Session activity and user-visible results are independent projections. Internal context compaction remains running activity, may supply `activityKind` / `activityLabel`, and stays in the Turn's technical detail; it does not create unread results or replace the last user-visible result. During context compaction the shared Composer suppresses steer and submits only to the next-Turn queue, while an ordinary running Turn retains steer and queue controls. Completion events are Turn-scoped, so a stale or internal completion cannot clear a newer active Turn, and positive authoritative activity keeps reconciliation alive until the Host confirms a terminal state.
- `SessionBrowser` and `SessionWorkspace` own common list, transcript, Composer, queue, approval, attachment, Subagent, Realtime and responsive interaction semantics. A Turn's collapsed technical-detail summary accepts normalized `startedAt` / `completedAt` metadata and presents elapsed duration rather than a wall-clock timestamp; an active Turn without `completedAt` updates an `已运行` duration once per second, while invalid or incomplete terminal timestamps omit the duration. Collapsing the Session list releases its full grid width and leaves a keyboard-focusable, edge-hover restore handle, so consumers do not need a layout override. A non-image attachment opened inside `SessionBrowser` places its modal backdrop over the complete Browser surface, including the Session list and collapse/restore handle, while the consumer page shell remains outside that layer; direct `SessionWorkspace` mounts retain their self-contained preview boundary. Composer compact controls respond to their mounted container's inline size rather than the browser viewport, so consumer sidebars do not squeeze or stretch settings and send actions.
- A `SessionReference` is a bounded, JSON-safe pointer whose identity is exactly `hostId + threadId`; title, context, update time and archive availability are display caches. `SessionBrowser` may expose an authorized reference as a dedicated drag MIME, while `SessionWorkspace` uses the same object for Host-provided `@` search, Composer chips, submission and transcript display. The Host owns search, pre-submit resolution, authorization, persistence, navigation and any read/write side effects. Platform never copies a referenced transcript into the prompt or treats a title as identity; absent Host actions leave existing Session and attachment behavior unchanged.
- The built-in Minimal Host bounds read amplification independently from durable Session history. Its initial list request returns 50 summaries and continues by opaque cursor. Selecting a Session returns its latest five projected Turns without technical payloads; history prepends ten earlier Turns per cursor while preserving the reader's visible-message anchor, and each Turn's technical items load only when its disclosure opens. Stored image descriptors remain path-free and resolve to revocable object URLs only near the viewport or on explicit click. Switching Sessions aborts outstanding detail, technical and media reads. Browser event bursts are coalesced to one state commit per animation frame, while the durable store may apply `item_delta` batches to its derived Turn projection and append-only journal without replaying or rewriting the full snapshot. The snapshot remains authoritative; the SQLite Turn projection is rebuildable after migration, stale state or restart and never changes Runtime, Edit, Fork, share or context semantics.
- Completed commentary remains grouped by Turn, but the newest completed process starts expanded so content visible during execution does not disappear at the completion boundary; disclosure state remains user-controlled. `messages[].media` is published-result media: a Host may populate it for user input and final Agent answers, but never by collecting observation, inspection, tool, Subagent, or commentary images. When Resources are enabled, the built-in Minimal Host promotes completed provider-native generated-image results through the transient-to-`session-artifact` lifecycle, removes raw image bytes and internal paths before Session persistence, and attaches the authorized Resource only to that Turn's final Agent message. A full consumer may similarly map generated-image results and images explicitly referenced by the final Agent message after Host authorization and durable promotion. Shared UI suppresses image markup and media galleries on commentary as a second boundary. Message-body images retain their aspect ratio and use bounded thumbnail dimensions. A host-resolved image link keeps the shared keyboard/click behavior and opens through the existing file action instead of granting Platform filesystem access. The Minimal Host resolves authorized user image attachments to short-lived object URLs and renders them as inline thumbnails inside the user's message; those thumbnails and full-consumer image attachments open in the same viewport-centered lightbox with a dimmed, blurred backdrop and bounded contain sizing. Non-image attachment drawers use the same blurred backdrop plus a translucent, independently blurred panel, with a solid reduced-transparency fallback. No image-specific feature flag is required beyond the existing Resource/attachment capability.
- Transcript auto-follow is conditional on reading intent. An upward wheel, touch gesture, or observed scroll-position decrease pauses following before the reader passes the general distance threshold; later streaming deltas preserve that viewport and surface the existing new-message shortcut. That shortcut is an overlay and never changes transcript height when it appears or disappears, including while the reader reaches the end of a completed response. Reaching the latest region or activating the shortcut resumes following. Explicit submission still follows the new user message, and viewport resize follows only until the user interrupts that submission-follow state.
...
- Session identity, transcript, draft and managed Resources are portable across Runs; Runtime provider threads, queues, credentials and workspace-private state remain Run-scoped. Reading an owned Session from an earlier Run is side-effect free and keeps the Composer available. Its first new Turn is serialized per Session, creates one fresh current-Run Runtime binding, and supplies bounded retained transcript as untrusted conversation context without copying the old provider thread, credentials, paths or technical state. Attachment and directory staging does not create a Runtime; managed Resources are reauthorized by Session when referenced by the new Turn. Failed Runtime preparation removes an incomplete current-Run binding while preserving the Session, draft, staged Resources and retry identity. Shared projections owned by another principal remain read-only.
- Edit and Fork create a new Session branch and never replace the source Session's Runtime binding. In the Minimal Host, a successful Edit archives the source Session so the owner sees only the replacement in the active list, while Fork keeps both Sessions active; archived sources remain available to the cross-owner Observer. A portable source without a current-Run Runtime still branches through bounded retained history instead of reusing a stale Runtime binding. Platform plans the branch and owns provider-neutral queue recovery; a consumer still owns authorization, storage adapters, product linking and navigation to the new Session.
- Minimal Host can consume a bounded, Gateway-verified shared-Session access envelope when the consumer configures owner and access headers. The base64url JSON envelope is versioned, its `principalId` must exactly match the verified owner, and it is limited to 100 grants and 16 KiB; a missing, malformed, oversized or mismatched configured envelope fails closed. The Gateway must clear browser-supplied identity/access claims before injecting its verified values.
- Shared projections expose only explicitly granted read-only transcripts/resources. Mutations and event streams are denied, managed-resource reads are reauthorized, and `session.fork` may create an idempotent new-owner continuation by copying visible history into a fresh Runtime without provider bindings, technical state or external workspace references. Consumers still own identity, Share/link state, revocation, audit, retention and request-scoped envelope paging.
...
- Session-reference drags use `application/x-agent-workbench-session-reference+json`, remain distinct from files/directories, and reject a mixed payload. A Turn accepts at most eight unique references, and `actions.onResolveSessionReferences` must return their currently authorized identities before `actions.onSubmit` receives them. Missing identities stay visible as unavailable references and cannot be silently rematched by title.
- Composer remains a direct text editor and never substitutes a formatted preview for the user's draft. Ordinary clipboard HTML wrappers, a standalone copied heading, and a single copied list item use the exact plain-text value; styling one editable line alone is not complex structure. By default, multi-block content, multi-item lists, tables, code blocks and other structurally complex clipboard content become an attachment through the existing staged Resource flow instead of adding a second editing mode. A consumer may set `attachmentPolicy.structuredTextPaste` to `inline` when its product prefers all short clipboard text to remain directly editable; length and Composer-limit conversion still apply, while clipboard files and images are unaffected.
- Generic managed-file inputs tell Runtime the exact authorized run-local Resource path as well as the display name; the display name is not assumed to exist in the workspace. That instruction remains inside the versioned attachment envelope, which the transcript presentation removes, while browser Session projections retain only the path-free Resource descriptor.
