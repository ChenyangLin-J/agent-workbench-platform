...
- `@agent-workbench/platform/session-client` supplies product-neutral browser application state. Its operation controller retains mutation identity for uncertain retries; its reconciliation helpers merge authoritative snapshots with optimistic/live items; and its event controller owns active-Turn, queue, request, delta, reconnect and polling transitions. Transport, authentication, product events, extension recovery, persistence and user-facing error policy remain consumer-owned callbacks.
- Session activity and user-visible results are independent projections. Internal context compaction remains running activity, may supply an activity label, and stays in the Turn's technical detail; it does not create unread results or replace the last user-visible result. Completion events are Turn-scoped, so a stale or internal completion cannot clear a newer active Turn, and positive authoritative activity keeps reconciliation alive until the Host confirms a terminal state.
