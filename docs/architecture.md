...
- Edit and Fork create a new Session branch and never replace the source Session's Runtime binding. In the Minimal Host, a successful Edit archives the source Session so the owner sees only the replacement in the active list, while Fork keeps both Sessions active; archived sources remain available to the cross-owner Observer. Platform plans the branch and owns provider-neutral queue recovery; a consumer still owns authorization, storage adapters, product linking and navigation to the new Session.
